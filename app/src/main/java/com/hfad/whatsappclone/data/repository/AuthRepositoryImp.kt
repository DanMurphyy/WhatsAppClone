...
class AuthRepositoryImp @Inject constructor(
    ...
    override fun createUserProfile(modelUser: ModelUser): Flow<Resource<Boolean>> =
        callbackFlow {
            try {
                trySend(Resource.Loading)
                firebaseFirestore.collection("users").document(getUserId()).set(modelUser)
                    .addOnSuccessListener {
                        trySend(Resource.Success<Boolean>(true))
                    }
                    .addOnFailureListener {
                        trySend(Resource.Error(it.localizedMessage ?: "An Error Occurred"))
